...
socketio = SocketIO(
    ...
    ping_interval=10,     # ✅ prevent false ping timeouts
    ping_timeout=25       # ✅ allow client to reconnect more gracefully
)
    ...
    try:
        ...
        if result["hands_detected"] > 0:
            if letter_classifier.is_trained and "letter_prediction" in result:
                ...
                print(f'👋 {result["hands_detected"]} hand(s) - {pred["letter"]} ({pred["confidence"]:.2%})')
            else:
                print(f'👋 {result["hands_detected"]} hand(s)')
...
if __name__ == "__main__":
    ...
    try:
        socketio.run(
            ...
            host="0.0.0.0",
            port=5001,
            debug=False,          # ✅ disable debug reload loop
            use_reloader=False    # ✅ prevents double-instance ping bug
        )
    finally:
